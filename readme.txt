...
로컬에서 실행할 때 
docker로 실행할 경우 branch docker를 이용할 것
프로젝트 구조
1. Nginx
...
주요 기능:
  Redis 연결 설정 및 관리
  메시지 캐싱 및 임시 저장 (최근 2000개)
  활성 연결 관리
  최근 메시지 조회 및 관리
  사용자별 최근 메시지 저장 (최근 200개)
...
주요 기능:
  실시간 메시지 캐싱
  활성 연결 관리
  최근 메시지 임시 저장 (전체 2000개, 사용자별 200개)
...
Kafka 관련 기능
JWT 토큰 관리:
  토큰 발급: 30분 만료 시간으로 설정
  토큰 갱신: 만료 5분 전에 자동 갱신 (사용자 활동 시)
  토큰 무효화: 로그아웃 또는 15분 무활동 시
유저 접속 정보 관리:
  로그인/로그아웃 이벤트 기록
  사용자 활동 추적 (채팅, 기타 상호작용)
  IP 주소, 디바이스 정보 등 저장
  자동 로그아웃 처리:
  15분 동안 활동이 없는 사용자 감지
  자동 로그아웃 이벤트 발생 및 처리
데이터 백업 및 동기화:
  Kafka에 저장된 사용자 접속 정보를 주기적으로 PostgreSQL에 백업
  실시간 데이터와 영구 저장소 간의 동기화 유지

추가 모달 창 기능
  초록색 닉네임을 클릭 가능한 버튼으로 변경
  버튼 클릭 시 모달 창 오픈
  모달 창 내용:
a. 가입 시 사용한 이메일 정보 표시
b. 최근 채팅 내역 20개 (Redis에서 가져옴)
c. 닉네임 변경 기능
d. 비밀번호 변경 기능

시작하기
redis kafka postgresql 설치 필요